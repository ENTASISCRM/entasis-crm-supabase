...
//   3. si aucun n est en poste (contrat termine, suite pas encore saisie), on
//      retombe sur le contrat marque actif, comme avant : rien ne disparait
//      tant que la direction n a pas saisi la suite.
//
// Le drapeau `actif` garde donc un role : exclusion manuelle explicite (une
// ligne de test, un contrat annule), et filet de securite. Il n est plus la
// seule verite.
...
export function contratDeReference(contrats, date = new Date()) {
  ...
  const actifs = liste.filter(c => c.actif)
