...
$academy_deck$, competence = coalesce(nullif($academy_deck$Préparer et structurer la découverte, l'analyse et le suivi$academy_deck$, ''), competence), updated_at = now() where id = v_ver;
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 22, $academy_deck$ordre$academy_deck$, $academy_deck$Préparer la découverte$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre les étapes du premier entretien de découverte.","elements":["Poser le cadre : « Je ne vous proposerai rien aujourd'hui. »","Accueillir, noter et reporter la demande de produit","Poser des questions ouvertes et reformuler","Résumer oralement et faire valider par le client","Annoncer l'analyse complète et une date de restitution","Consigner l'échange dans la fiche client"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3,4,5]}$academy_deck$::jsonb, $academy_deck$Cadre, demande notée et reportée, questions ouvertes, résumé validé, annonce de la suite, puis traçage dans la fiche client.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 23, $academy_deck$ordre$academy_deck$, $academy_deck$Structurer l'analyse$academy_deck$, 1, $academy_deck${"enonce":"Remettez dans l'ordre les étapes de l'étude selon la méthode Entasis.","elements":["Le diagnostic : où en est le client ?","Les objectifs de vie hiérarchisés, avec leur horizon","La stratégie : comment y aller ?","Les moyens : les solutions au service d'un objectif"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3]}$academy_deck$::jsonb, $academy_deck$L'étude suit un ordre : diagnostic, objectifs hiérarchisés, stratégie, puis moyens ; les solutions n'apparaissent qu'en dernier.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 24, $academy_deck$ordre$academy_deck$, $academy_deck$Argumenter et chiffrer$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre le déroulé du rendez-vous de restitution de l'étude.","elements":["Présenter le diagnostic : « Voici ce que j'ai compris de votre situation », et le faire valider","Exposer les objectifs hiérarchisés identifiés ensemble","Présenter la stratégie et les moyens, avec objectif servi, chiffre, contrainte, risque et scénario défavorable","Conclure par les décisions à prendre, les documents à réunir et la prochaine étape"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3]}$academy_deck$::jsonb, $academy_deck$La restitution suit l'ordre de l'étude : diagnostic validé, objectifs hiérarchisés, stratégie et moyens, puis décisions et prochaine étape.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 25, $academy_deck$ordre$academy_deck$, $academy_deck$Organiser le suivi$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre la préparation et la conduite du point annuel.","elements":["Relire la feuille de route et les échanges de l'année","Ouvrir par : « Qu'est-ce qui a changé dans votre vie depuis notre dernier point ? »","Passer en revue famille, activité, revenus, projets, rapport au risque","Reprendre chaque brique en place et vérifier qu'elle sert toujours son objectif","Fixer la prochaine échéance","Tracer l'échange dans le CRM le jour même"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3,4,5]}$academy_deck$::jsonb, $academy_deck$Le point annuel se prépare par relecture, s'ouvre sur les changements, revoit chaque brique, puis fixe l'échéance et se trace le jour même.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 26, $academy_deck$association$academy_deck$, $academy_deck$Préparer la découverte$academy_deck$, 1, $academy_deck${"enonce":"Associez chaque temps de la méthode Entasis à son contenu.","gauche":["L'écoute","L'étude","L'accompagnement"],"droite":["Premier entretien de 30 minutes pour comprendre, sans proposer","Analyse patrimoniale complète et feuille de route argumentée","Conseiller dédié, point annuel formel, ajustements au fil de la vie"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2]]}$academy_deck$::jsonb, $academy_deck$La méthode tient en trois temps : l'écoute pour comprendre, l'étude pour argumenter, l'accompagnement pour ajuster dans la durée.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 27, $academy_deck$association$academy_deck$, $academy_deck$Structurer l'analyse$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque question de l'étude à l'étape qui y répond.","gauche":["Où en est le client ?","Où veut-il aller ?","Comment y aller ?"],"droite":["Le diagnostic","Les objectifs de vie hiérarchisés","La stratégie"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2]]}$academy_deck$::jsonb, $academy_deck$Le diagnostic décrit la situation, les objectifs hiérarchisés fixent la destination, la stratégie dit comment y aller.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 28, $academy_deck$association$academy_deck$, $academy_deck$Préparer la découverte$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque phrase du conseiller au moment où elle se dit.","gauche":["« Je ne vous proposerai rien aujourd'hui. »","« C'est noté, nous l'examinerons dans l'étude. »","« Voici ce que j'ai compris de votre situation. »","« Qu'est-ce qui a changé dans votre vie depuis notre dernier point ? »"],"droite":["Ouverture du premier entretien","Réponse à une demande de produit en découverte","Ouverture du rendez-vous de restitution","Ouverture du point annuel"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$Chaque temps de la méthode a sa phrase d'ouverture : le cadre, le report de la demande, le diagnostic à valider, la question des changements.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 29, $academy_deck$association$academy_deck$, $academy_deck$Recueillir avant de conseiller$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque information à recueillir à la question ouverte qui la fait émerger.","gauche":["Ce qui amène le client","L'objectif de l'épargne","L'horizon","La tolérance au risque"],"droite":["« Qu'est-ce qui vous amène ? »","« À quoi doit servir cette épargne ? »","« Dans combien de temps ? »","« Comment réagiriez-vous si la valeur baissait ? »"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$Les questions ouvertes du premier entretien recueillent projets, horizon et rapport au risque sans évoquer de solution.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
