...
- Trois conditions : fraction limitée d'un patrimoine diversifié, horizon au moins égal à la période recommandée, perte supportable.$academy_deck$, competence = coalesce(nullif($academy_deck$Relier diversification, volatilité, horizon et capacité de perte$academy_deck$, ''), competence), updated_at = now() where id = v_ver;
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 22, $academy_deck$ordre$academy_deck$, $academy_deck$Méthode et traçabilité$academy_deck$, 2, $academy_deck${"enonce":"Cas fictif. Une cliente veut arbitrer tout son contrat vers un fonds sectoriel qui vient de monter. Remettez dans l'ordre la démarche du conseiller.","elements":["Tracer dans le recueil d'informations et la déclaration d'adéquation écrite","Étudier par poche datée","Expliquer sans promettre : perte en capital, volatilité, frais, performance passée","Écouter et reformuler l'objectif réel"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[3,1,2,0]}$academy_deck$::jsonb, $academy_deck$Ni refus sec ni exécution : on écoute, on étudie, on explique, puis on formalise la recommandation dans la déclaration d'adéquation.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 23, $academy_deck$ordre$academy_deck$, $academy_deck$Diversification d'ensemble$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre les étapes du rendez-vous pour repérer une concentration cachée.","elements":["Ouvrir les documents des fonds déjà détenus","Nommer les expositions dominantes","Proposer ce qui les complète","Dresser la carte complète du patrimoine, y compris ce qui ne se place pas"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[3,0,1,2]}$academy_deck$::jsonb, $academy_deck$On regarde tout, on lit ce que contiennent les fonds, on nomme les expositions, puis on propose ce qui complète.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 24, $academy_deck$ordre$academy_deck$, $academy_deck$Horizon du projet$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l’ordre la lecture d’un support avec le client, avant de le retenir pour un projet.","elements":["Montrer le scénario défavorable et le scénario de tension","Lire la période de détention recommandée et la comparer à la date du projet","Lire l'indicateur de risque du DIC","Dater le projet : « cette somme, à quelle date en aurez-vous besoin ? »"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[3,2,1,0]}$academy_deck$::jsonb, $academy_deck$On date le projet avant de parler de support, puis on lit le DIC : indicateur, période recommandée, scénarios.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 25, $academy_deck$ordre$academy_deck$, $academy_deck$Tolérance et capacité$academy_deck$, 3, $academy_deck${"enonce":"Cas fictif. Un jeune professionnel endetté, sans réserve, aux revenus dépendant de sa seule activité, se dit très à l’aise avec le risque et veut « du rendement ». Remettez dans l’ordre sa feuille de route.","elements":["Constater que sa capacité de perte est faible malgré la tolérance déclarée","Envisager une allocation exposée, le rendement venant sur un horizon qui le permet","Constituer une réserve disponible","Protéger ses revenus"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,3,2,1]}$academy_deck$::jsonb, $academy_deck$La capacité prime sur la tolérance : revenus protégés d’abord, puis réserve disponible, avant toute allocation exposée.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 26, $academy_deck$association$academy_deck$, $academy_deck$Tolérance et capacité$academy_deck$, 1, $academy_deck${"enonce":"Associez chaque notion à sa définition.","gauche":["Tolérance au risque","Capacité de perte","Volatilité","Perte en capital"],"droite":["Amplitude des variations d'un placement","Ce qu'il peut perdre sans compromettre ses projets","Peut être totale lorsqu'elle existe","Ce que le client accepte de voir bouger"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,3],[1,1],[2,0],[3,2]]}$academy_deck$::jsonb, $academy_deck$La tolérance décrit le rapport au risque, la capacité la situation financière ; volatilité et perte en capital ne se confondent pas.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 27, $academy_deck$association$academy_deck$, $academy_deck$Lecture du DIC$academy_deck$, 1, $academy_deck${"enonce":"Associez chaque élément du DIC à ce qu'il indique.","gauche":["Indicateur de risque","Période de détention recommandée","Scénario de tension"],"droite":["Évolution extrême, pas un plancher garanti","Durée que suppose le niveau de risque affiché","Échelle de 1 à 7, risque de marché et de crédit combinés"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,2],[1,1],[2,0]]}$academy_deck$::jsonb, $academy_deck$L'indicateur note de 1 à 7, il suppose la détention sur la période recommandée, et le scénario de tension ne garantit rien.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 28, $academy_deck$association$academy_deck$, $academy_deck$Diversification d'ensemble$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque situation au terme qui la désigne.","gauche":["Cinq fonds investis sur les mêmes grandes valeurs technologiques","Répartir à parts égales entre les options présentées sans regarder leur contenu","Imiter les choix de son entourage","Récession, hausse des taux, crise"],"droite":["Comportement moutonnier","Une seule position","Risque commun à tout le marché","Diversification naïve"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,1],[1,3],[2,0],[3,2]]}$academy_deck$::jsonb, $academy_deck$L'AMF nomme la diversification naïve et le comportement moutonnier ; le risque de marché touche presque tout en même temps.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 29, $academy_deck$association$academy_deck$, $academy_deck$Horizon du projet$academy_deck$, 3, $academy_deck${"enonce":"Cas fictifs. Associez chaque poche d'épargne à la lecture qui convient.","gauche":["35 000 euros pour acheter ses bureaux dans trois ans","Complément de revenus à partir de 64 ans, pour une cliente de 44 ans","40 000 euros pour sa fille dans deux ans, face à un fonds recommandé cinq ans","Poche retraite d'une cliente de 51 ans qui cesse son activité à 62 ans"],"droite":["Une part d'unités de compte diversifiées, risque expliqué et accepté","Supports peu volatils, potentiel modeste accepté","Incompatible : l'horizon est plus court que la période recommandée","Environ onze ans : une part d'unités de compte possible, pas un secteur unique"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,1],[1,0],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$Chaque poche se lit à sa date : peu de volatilité à court terme, une part d'unités de compte diversifiées à long terme.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
