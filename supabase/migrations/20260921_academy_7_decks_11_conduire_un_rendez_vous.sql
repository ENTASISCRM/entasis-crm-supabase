...
$academy_deck$, competence = coalesce(nullif($academy_deck$Reformuler, expliquer les frais, répondre sans promettre et conclure une prochaine étape$academy_deck$, ''), competence), updated_at = now() where id = v_ver;
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 22, $academy_deck$ordre$academy_deck$, $academy_deck$Reformuler une objection$academy_deck$, 1, $academy_deck${"enonce":"Remettez dans l'ordre le traitement d'une objection, de son apparition à la réponse.","elements":["Faire préciser : « Concrètement, que feriez-vous seul ? »","Reformuler : « Si je vous comprends bien... C'est bien cela ? »","Répondre, après la confirmation du client","Accueillir : « C'est une question légitime. »"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[3,1,0,2]}$academy_deck$::jsonb, $academy_deck$Accueillir, reformuler, faire préciser ; la réponse ne vient qu'après la confirmation du client.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 23, $academy_deck$ordre$academy_deck$, $academy_deck$Familles et étages de frais$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre la présentation des frais en rendez-vous, de la préparation à la recommandation.","elements":["Préparer les documents d'informations clés et repérer la rubrique frais","Présenter les frais du contrat et des supports, en euros sur le versement réel","Demander : « Est-ce clair pour vous ? Souhaitez-vous que je vous l'adresse par écrit ? »","Formuler la recommandation","Terminer par la rémunération du cabinet et ce qu'elle comprend"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,4,2,3]}$academy_deck$::jsonb, $academy_deck$Les frais se présentent avant la recommandation, pas après, et la rémunération du cabinet clôt la présentation.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 24, $academy_deck$ordre$academy_deck$, $academy_deck$Répondre sans promettre$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre les temps d'une réponse honnête à une objection, jusqu'à la conclusion.","elements":["Proposer une prochaine étape datée","Décrire ce que le cabinet apporte","Reconnaître la part de vérité de l'objection","Nommer les risques et les limites"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[2,1,3,0]}$academy_deck$::jsonb, $academy_deck$Une réponse honnête tient en trois temps : part de vérité, apport du cabinet, risques et limites, puis l'étape suivante.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 25, $academy_deck$ordre$academy_deck$, $academy_deck$Conclure sur une étape$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre la conclusion d'un rendez-vous, de l'agenda à la trace écrite.","elements":["Faire valider la prochaine étape par le client","La saisir dans le CRM dès la fin du rendez-vous, avec l'objection reformulée et la réponse donnée","La relire à voix haute avant de se quitter","Proposer une action, une date et un livrable"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[3,0,2,1]}$academy_deck$::jsonb, $academy_deck$Proposer, faire valider, relire à voix haute, puis saisir dans le CRM dès la fin du rendez-vous.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 26, $academy_deck$association$academy_deck$, $academy_deck$Reformuler une objection$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque geste de reformulation à la phrase qui l'illustre.","gauche":["Reformulation qui déforme","Reformulation qui oriente","Reformulation fidèle","Question de précision"],"droite":["« Vous voulez donc un conseil personnalisé, n'est-ce pas ? »","« Vous trouvez que c'est trop cher », alors que le client parlait de valeur ajoutée","« Si je vous comprends bien... C'est bien cela ? »","« Concrètement, que feriez-vous seul ? »"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,1],[1,0],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$Déformer traduit le client, orienter déguise une réponse ; la reformulation fidèle demande validation avant une seule question.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 27, $academy_deck$association$academy_deck$, $academy_deck$Familles et étages de frais$academy_deck$, 1, $academy_deck${"enonce":"Associez chaque frais d'assurance vie au moment où il est prélevé.","gauche":["Frais sur versement","Frais de gestion","Frais d'arbitrage","Frais de retrait"],"droite":["À la sortie, quand le client retire","Chaque année","À chaque transfert entre supports","À l'entrée, sur chaque versement"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,3],[1,1],[2,2],[3,0]]}$academy_deck$::jsonb, $academy_deck$Versement et retrait sont ponctuels, la gestion est annuelle, l'arbitrage suit chaque transfert entre supports.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 28, $academy_deck$association$academy_deck$, $academy_deck$Lettre de mission et textes$academy_deck$, 3, $academy_deck${"enonce":"Associez chaque texte à ce qu'il établit.","gauche":["Article 325-6 du règlement général de l'AMF","Article L541-8-1 du code monétaire et financier","Article L521-4 du code des assurances","Article L522-5 du code des assurances","Recommandation ACPR 2024-R-03"],"droite":["Pour les unités de compte, performance brute de frais, nette de frais et frais prélevés","Devoir de conseil dans la durée pour les contrats d'assurance vie","Le CIF agit de manière honnête, loyale et professionnelle ; information exacte, claire et non trompeuse","Lettre de mission signée des deux parties avant tout conseil du CIF","Exigences et besoins du client précisés par écrit avant tout contrat d'assurance"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,3],[1,2],[2,4],[3,0],[4,1]]}$academy_deck$::jsonb, $academy_deck$Chaque texte porte une obligation distincte : lettre de mission, loyauté et information, écrit préalable, transparence des UC, conseil dans la durée.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 29, $academy_deck$association$academy_deck$, $academy_deck$Répondre sans promettre$academy_deck$, 3, $academy_deck${"enonce":"Associez chaque famille de phrase à proscrire à l'exemple qui l'illustre.","gauche":["Promesse de résultat","Superlatif","Garantie implicite","Performance passée sans mise en garde"],"droite":["« C'est sécurisé. »","« Vous ferez mieux qu'en solo. »","« Nos fonds ont fait 7 % l'an dernier, cela parle de soi-même. »","« C'est le meilleur contrat. »"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,1],[1,3],[2,0],[3,2]]}$academy_deck$::jsonb, $academy_deck$Promesse, superlatif et garantie implicite sont interdits ; une performance passée exige la mention sur les performances futures et le risque.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
