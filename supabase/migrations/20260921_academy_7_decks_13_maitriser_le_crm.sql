...
$academy_deck$, competence = coalesce(nullif($academy_deck$Tenir une fiche, tracer un échange, programmer le suivi et utiliser les étapes réellement présentes dans le CRM$academy_deck$, ''), competence), updated_at = now() where id = v_ver;
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 22, $academy_deck$ordre$academy_deck$, $academy_deck$Créer un dossier$academy_deck$, 2, $academy_deck${"enonce":"Cas fictif. Vous raccrochez avec une prospecte intéressée par un PER. Remettez dans l'ordre la création express du dossier.","elements":["Poser la « Date de signature prévue »","Le soir, rouvrir le dossier pour la compagnie et la PP mensuelle","Cliquer « Créer le dossier »","Rechercher un client existant, sinon saisir nom, email, téléphone et mois","Choisir « Produit * » (PER Individuel)","Touche N : la modale s'ouvre en mode express"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[5,3,4,0,2,1]}$academy_deck$::jsonb, $academy_deck$L'express suffit pour commencer : identité, au moins un produit et une date prévue, puis on complète plus tard.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 23, $academy_deck$ordre$academy_deck$, $academy_deck$Programmer le suivi$academy_deck$, 1, $academy_deck${"enonce":"Remettez dans l'ordre d'affichage les blocs de l'accueil « Mon mois », du haut vers le bas.","elements":["« Ma journée » : les relances en retard","Plus bas, le bloc « Dossiers sans mouvement »","« Ma journée » : les relances du jour","« Ma journée » : les RDV du jour"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[3,0,2,1]}$academy_deck$::jsonb, $academy_deck$« Ma journée » affiche RDV du jour, relances en retard puis relances du jour ; « Dossiers sans mouvement » vient plus bas.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 24, $academy_deck$ordre$academy_deck$, $academy_deck$Programmer le suivi$academy_deck$, 2, $academy_deck${"enonce":"Cas fictif. Lundi, vous envoyez un devis d'assurance vie. Remettez dans l'ordre le déroulé d'une séquence « Relance devis standard ».","elements":["Choisir le gabarit « Relance devis standard » et cliquer « Démarrer »","Ouvrir le dossier en formulaire complet, bloc « Séquence de relance »","Le jour venu, appeler le client depuis « Ma journée »","Cliquer « Enregistrer » pour persister l'étape 1","L'étape 2 s'arme sept jours après le clic","Consigner l'appel dans « Historique » puis cliquer « Fait »"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[1,0,3,2,5,4]}$academy_deck$::jsonb, $academy_deck$« Démarrer » pose l'étape 1, « Enregistrer » la persiste, « Fait » arme l'étape suivante datée depuis aujourd'hui.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 25, $academy_deck$ordre$academy_deck$, $academy_deck$Signer proprement$academy_deck$, 3, $academy_deck${"enonce":"Cas fictif. Monsieur Ravel a signé vendredi. Remettez dans l'ordre les gestes d'une signature propre.","elements":["Si le toast liste des champs manquants, les remplir puis enregistrer de nouveau","Poser la « Date de signature effective »","Glisser la carte vers « Signé ✓ » : la modale s'ouvre en Signé","Cliquer « Enregistrer »","Vérifier « Signature n/6 » sur la fiche client"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[4,2,1,3,0]}$academy_deck$::jsonb, $academy_deck$La signature passe par la modale ; le toast « Pour signer, complétez la fiche client : … » nomme ce qui manque.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 26, $academy_deck$association$academy_deck$, $academy_deck$Programmer le suivi$academy_deck$, 1, $academy_deck${"enonce":"Associez chaque gabarit de séquence de relance à ses étapes.","gauche":["« Relance devis standard »","« Suite de rendez vous »","« Pièces manquantes »"],"droite":["J+1, J+5, J+12","J+2, J+7, J+15","J+3, J+10, J+20"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,1],[1,0],[2,2]]}$academy_deck$::jsonb, $academy_deck$Devis standard J+2, J+7, J+15 ; suite de rendez vous J+1, J+5, J+12 ; pièces manquantes J+3, J+10, J+20.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 27, $academy_deck$association$academy_deck$, $academy_deck$Créer un dossier$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque statut de dossier à ce qui le caractérise.","gauche":["En cours","RDV calé","Signé","Annulé"],"droite":["Dossier ouvert, « Date de signature prévue » obligatoire","Colonne « Signé ✓ », « Date de signature effective » obligatoire","Issue de « Abandonner » ou de « No-show »","Valeur « Prévu », brouillon posé par la Lead Room"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,3],[2,1],[3,2]]}$academy_deck$::jsonb, $academy_deck$Quatre statuts : En cours et RDV calé exigent la date prévue, Signé la date effective, Annulé vient d'un abandon ou d'un no-show.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 28, $academy_deck$association$academy_deck$, $academy_deck$Trier sans mouvement$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque geste de « Dossiers sans mouvement » à son effet.","gauche":["« Déjà signé »","« Toujours en cours »","« Relancer »","« Abandonner »"],"droite":["Ouvre le dossier pour y poser une date","Passe le dossier en Annulé après confirmation","Ouvre la modale en Signé","Le dossier se tait 21 jours de plus"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,2],[1,3],[2,0],[3,1]]}$academy_deck$::jsonb, $academy_deck$Quatre gestes : signer, faire taire 21 jours, dater une relance, ou annuler après confirmation.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 29, $academy_deck$association$academy_deck$, $academy_deck$Compléter la fiche client$academy_deck$, 3, $academy_deck${"enonce":"Associez chaque indicateur du CRM à ce qu'il mesure.","gauche":["Verrou de signature","Jauge de complétude","Puce « À compléter »","« Signés, fiche à finir »"],"droite":["Six champs, affichés « Signature n/6 »","Huit champs pondérés, quatre niveaux","Dossier Signé, jauge sous 100 %, plus de 21 jours sans mouvement","Produit Autre et PP mensuelle à 0"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,3],[3,2]]}$academy_deck$::jsonb, $academy_deck$Le verrou dit le droit de signer, la jauge la qualité de la donnée ; la puce signale un brouillon.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
