...
$academy_deck$, competence = coalesce(nullif($academy_deck$Expliquer mécanisme, disponibilité, fiscalité à vérifier et arbitrages$academy_deck$, ''), competence), updated_at = now() where id = v_ver;
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 22, $academy_deck$ordre$academy_deck$, $academy_deck$Ordre des priorités$academy_deck$, 1, $academy_deck${"enonce":"Remettez dans l'ordre les étapes de la feuille de route d'un client sans prévoyance ni épargne disponible qui veut verser sur un PER.","elements":["Protéger les revenus par un audit de la prévoyance","Constituer une épargne disponible sur un support liquide","Isoler les projets de moyen terme sur une enveloppe disponible, distincte du PER","Ouvrir un PER avec des versements programmés calibrés sur la capacité d'épargne réelle","Choisir le mode de gestion à partir du rapport au risque"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3,4]}$academy_deck$::jsonb, $academy_deck$Protection des revenus, épargne disponible, projets isolés, puis PER calibré en dernier et choix du mode de gestion.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 23, $academy_deck$ordre$academy_deck$, $academy_deck$Sortie et gestion pilotée$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre la présentation du PER en rendez-vous, telle que l'enseigne ce parcours.","elements":["Énoncer le blocage des sommes jusqu'à la retraite, sauf cas prévus par la loi","Demander si ce blocage est compatible avec les projets des cinq prochaines années","Présenter les deux modes de sortie, capital ou complément de revenu","Expliquer la gestion pilotée et le risque de perte en capital des unités de compte","Aborder seulement ensuite l'avantage fiscal"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3,4]}$academy_deck$::jsonb, $academy_deck$Le conseiller commence par la disponibilité, puis la sortie, puis le risque, avant tout avantage fiscal.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 24, $academy_deck$ordre$academy_deck$, $academy_deck$Déduction et fiscalité de sortie$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre chronologique le parcours fiscal d'un versement volontaire déduit.","elements":["Versement volontaire effectué avant 70 ans","Déduction du revenu imposable dans la limite du plafond","Blocage des sommes jusqu'à la retraite","Sortie en capital ou en rente","Imposition de la part correspondante au barème progressif ou comme une pension"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3,4]}$academy_deck$::jsonb, $academy_deck$La déduction à l'entrée reporte l'imposition au moment de la sortie, elle ne l'efface pas.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 25, $academy_deck$ordre$academy_deck$, $academy_deck$Ordre des priorités$academy_deck$, 3, $academy_deck${"enonce":"Cas fictif. Un indépendant dit « je veux payer moins d'impôt ». Remettez dans l'ordre la conduite du rendez-vous enseignée dans ce parcours.","elements":["Reformuler la demande et demander ce qui se passerait si l'activité s'arrêtait trois mois","Faire apparaître la réserve disponible, la couverture prévoyance et les échéances à venir","Présenter la répartition : une part disponible, une part de moyen terme, une part longue sur le PER","Fixer une prochaine étape datée : l'envoi de l'étude chiffrée avec les deux scénarios de déduction","Tenir un second rendez-vous pour décider"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3,4]}$academy_deck$::jsonb, $academy_deck$L'écoute précède l'étude, l'étude précède la solution, et la décision attend le second rendez-vous.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 26, $academy_deck$association$academy_deck$, $academy_deck$Choix des enveloppes$academy_deck$, 1, $academy_deck${"enonce":"Associez chaque enveloppe à ce qu'elle apporte.","gauche":["PER","Assurance vie","Livret réglementé"],"droite":["Déduction à l'entrée et blocage jusqu'à la retraite","Disponibilité de l'épargne et fiscalité de capitalisation, sans déduction","Liquidité immédiate et sécurité du capital"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2]]}$academy_deck$::jsonb, $academy_deck$Chaque enveloppe répond à un objectif de vie différent : retraite, projet à moyen terme, sécurité.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 27, $academy_deck$association$academy_deck$, $academy_deck$Déduction et fiscalité de sortie$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque situation de sortie du PER à son traitement fiscal.","gauche":["Capital issu de versements déduits","Capital issu de versements non déduits","Rente issue de versements déduits","Gains du plan"],"droite":["Imposé au barème progressif","Exonéré","Règles des pensions de retraite","Régime distinct, seuil à vérifier sur la source officielle"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$Le choix de déduire ou non à l'entrée commande la fiscalité de sortie.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 28, $academy_deck$association$academy_deck$, $academy_deck$Plafond de déduction$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque règle du plafond de déduction à son contenu.","gauche":["Report du plafond non utilisé","Mutualisation des plafonds","Année des revenus pris en compte","Revenus retenus pour un salarié"],"droite":["Sur les trois années suivantes","Couples mariés ou pacsés, sur demande expresse","L'année précédant le versement","Revenus d'activité nets de frais professionnels"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$Le plafond se lit sur l'avis d'imposition, reports compris, et se vérifie sur la source officielle datée.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 29, $academy_deck$association$academy_deck$, $academy_deck$Blocage et déblocage anticipé$academy_deck$, 3, $academy_deck${"enonce":"Associez chaque cas de déblocage anticipé du PER à la condition qui l'encadre.","gauche":["Achat de la résidence principale","Cessation de la fonction de mandataire social","Liquidation judiciaire","Invalidité"],"droite":["Versements volontaires et épargne salariale seulement","Depuis au moins deux ans, sans contrat de travail ni pension liquidée","Pour un non salarié","Deuxième ou troisième catégorie"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$Chaque cas de déblocage anticipé est encadré par une condition précise fixée par la loi.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
