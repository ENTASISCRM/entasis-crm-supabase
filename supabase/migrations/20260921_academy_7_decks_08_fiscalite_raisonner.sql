...
- Tableau avec et sans, jusqu'à la sortie ; relier à un objectif de vie, sinon bilan patrimonial.$academy_deck$, competence = coalesce(nullif($academy_deck$Distinguer déduction, réduction, crédit et logique économique$academy_deck$, ''), competence), updated_at = now() where id = v_ver;
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 22, $academy_deck$ordre$academy_deck$, $academy_deck$Bilan économique complet$academy_deck$, 1, $academy_deck${"enonce":"Remettez dans l'ordre les cinq questions à poser avant de proposer un dispositif à avantage fiscal.","elements":["Que coûte l'opération ?","L'avantage peut-il être repris ?","Combien de temps l'argent est-il immobilisé ?","Que se passe-t-il à la sortie ?","L'épargne de précaution est-elle intacte ?"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,2,3,1,4]}$academy_deck$::jsonb, $academy_deck$Coût, durée, sortie, reprise possible, épargne de précaution : les cinq questions se posent dans cet ordre.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 23, $academy_deck$ordre$academy_deck$, $academy_deck$Lecture de l'avis d'imposition$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre les étapes pour estimer en rendez-vous l'effet d'un versement déductible.","elements":["Situer la tranche marginale du client","Demander le dernier avis d'imposition","Reporter la simulation dans la feuille de route avec la source et la date","Vérifier le plafond du dispositif, puis le plafonnement global s'il s'applique","Estimer l'économie en disant « environ » et en précisant la condition de tranche","Repérer le revenu imposable, l'impôt dû et le plafond épargne retraite"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[1,5,0,4,3,2]}$academy_deck$::jsonb, $academy_deck$On part de l'avis, on situe la TMI, on estime avec « environ », on vérifie les plafonds, puis on consigne avec la source.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 24, $academy_deck$ordre$academy_deck$, $academy_deck$Trois mécanismes fiscaux$academy_deck$, 1, $academy_deck${"enonce":"Remettez dans l'ordre les étapes du calcul qui font intervenir déduction, réduction et crédit.","elements":["Rembourser le surplus éventuel de crédit d'impôt","Imputer les réductions et crédits sur l'impôt calculé","Retirer la déduction du revenu imposable","Calculer l'impôt sur le revenu restant, tranche par tranche","Déclarer le revenu"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[4,2,3,1,0]}$academy_deck$::jsonb, $academy_deck$La déduction intervient avant le calcul de l'impôt ; réduction et crédit s'imputent après, et seul le surplus de crédit est remboursé.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 25, $academy_deck$ordre$academy_deck$, $academy_deck$Bilan économique complet$academy_deck$, 3, $academy_deck${"enonce":"Cas fictif. Un prospect veut « juste savoir combien il économise » avec un Girardin. Remettez dans l'ordre votre réponse en rendez-vous.","elements":["Reformuler : il veut que son impôt serve à quelque chose plutôt que de le subir","Comparer dans la feuille de route plusieurs trajectoires jusqu'à la sortie, chiffrées et sourcées","Proposer un bilan patrimonial avec l'avis d'imposition complet et le prévisionnel de ses projets","Nommer les contraintes aussi clairement que l'avantage : apport perdu, reprise possible, plafonnement, liquidité"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,3,2,1]}$academy_deck$::jsonb, $academy_deck$Reformulation, contraintes nommées, bilan patrimonial, puis feuille de route comparée sur la durée : aucune souscription en fin de rendez-vous.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 26, $academy_deck$association$academy_deck$, $academy_deck$Trois mécanismes fiscaux$academy_deck$, 1, $academy_deck${"enonce":"Associez chaque mécanisme à ce qu'il vaut réellement pour le client.","gauche":["Déduction","Réduction d'impôt","Crédit d'impôt"],"droite":["Sa valeur faciale, surplus remboursé","Au plus l'impôt dû","Ce que vaut la tranche du client"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,2],[1,1],[2,0]]}$academy_deck$::jsonb, $academy_deck$Une déduction vaut la tranche, une réduction au plus l'impôt dû, un crédit sa valeur faciale.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 27, $academy_deck$association$academy_deck$, $academy_deck$Sortie, blocage et reprise$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque avantage fiscal à ce qui l'attend ensuite.","gauche":["Déduction des versements sur un PER","Réduction d'impôt Girardin","Réduction d'impôt supérieure à l'impôt dû"],"droite":["Gain à fonds perdus, reprenable si les conditions d'exploitation cessent","Report d'imposition : les versements déduits sont imposés à la sortie","Surplus perdu, sauf report prévu par le dispositif"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,1],[1,0],[2,2]]}$academy_deck$::jsonb, $academy_deck$Le PER reporte l'imposition, le Girardin est un gain à fonds perdus reprenable, la réduction excédentaire est perdue.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 28, $academy_deck$association$academy_deck$, $academy_deck$Plafonds et plafonnement global$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque plafond ou seuil à sa valeur pour l'imposition 2026 des revenus 2025.","gauche":["Plafond épargne retraite","Plafonnement global de droit commun","Plafonnement global avec outre-mer ou Sofica","Seuil de restitution du surplus d'un crédit d'impôt"],"droite":["8 €","10 000 €","Imprimé sur l'avis : année en cours plus reliquats des trois années précédentes","18 000 €"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,2],[1,1],[2,3],[3,0]]}$academy_deck$::jsonb, $academy_deck$Plafond épargne retraite sur l'avis, plafonnement global 10 000 € ou 18 000 €, restitution du crédit au-delà de 8 €.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 29, $academy_deck$association$academy_deck$, $academy_deck$Effet réel selon la TMI$academy_deck$, 2, $academy_deck${"enonce":"Cas fictifs. Associez chaque versement déductible, resté dans la tranche, à l'économie d'impôt approximative.","gauche":["TMI 11 %, versement de 6 000 €","TMI 30 %, versement de 6 000 €","TMI 41 %, versement de 10 000 €"],"droite":["Environ 4 100 €","Environ 660 €","Environ 1 800 €"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,1],[1,2],[2,0]]}$academy_deck$::jsonb, $academy_deck$L'économie vaut environ le versement multiplié par la TMI tant qu'il reste dans la tranche.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
