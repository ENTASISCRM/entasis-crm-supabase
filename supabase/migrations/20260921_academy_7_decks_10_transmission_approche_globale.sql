...
- Accord écrit avant tout envoi à un tiers. Aucune validation orale d'un schéma non lu.$academy_deck$, competence = coalesce(nullif($academy_deck$Identifier les objectifs familiaux et les relais professionnels$academy_deck$, ''), competence), updated_at = now() where id = v_ver;
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 4, $academy_deck$ordre$academy_deck$, $academy_deck$Objectifs familiaux$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre les questions à poser en rendez-vous avant de nommer un outil.","elements":["« Y a-t-il un bien que vous voulez voir aller à une personne précise ? »","« Comment voyez-vous l'équilibre entre vos enfants ? »","« Qui souhaitez-vous protéger en priorité ? »","Terminer par la situation civile complète : mariage ou PACS, régime matrimonial, enfants d'une première union","« De quoi avez-vous besoin pour vivre sereinement jusqu'à la fin ? »"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[2,0,1,4,3]}$academy_deck$::jsonb, $academy_deck$Quatre questions dans cet ordre, chacune reformulée et écrite, puis la situation civile ; aucun outil avant.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 9, $academy_deck$association$academy_deck$, $academy_deck$Réserve héréditaire$academy_deck$, 1, $academy_deck${"enonce":"Associez le nombre d'enfants à la réserve héréditaire correspondante.","gauche":["Un enfant","Deux enfants","Trois enfants et plus"],"droite":["Les deux tiers des biens","La moitié des biens","Les trois quarts des biens"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,1],[1,0],[2,2]]}$academy_deck$::jsonb, $academy_deck$Réserve des enfants : moitié, deux tiers, trois quarts selon leur nombre ; le reste est la quotité disponible.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 19, $academy_deck$association$academy_deck$, $academy_deck$Bilan préalable$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque pièce demandée au volet du bilan qu'elle renseigne.","gauche":["Contrat de mariage ou convention de PACS","Actes des donations antérieures","Relevés et clauses des contrats","Statuts de société"],"droite":["Donations déjà faites","Entreprise","Assurance vie","Situation civile et régime matrimonial"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,3],[1,0],[2,2],[3,1]]}$academy_deck$::jsonb, $academy_deck$Chaque pièce renseigne un volet ; chiffrer sans les avoir donne un chiffre faux qui engage la parole du cabinet.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 26, $academy_deck$association$academy_deck$, $academy_deck$Donations et abattements$academy_deck$, 3, $academy_deck${"enonce":"Associez chaque situation au traitement fiscal des capitaux d'assurance vie au décès.","gauche":["Primes versées avant 70 ans","Primes versées après 70 ans","Conjoint marié ou partenaire de PACS bénéficiaire"],"droite":["Exonération","Abattement de 152 500 euros par bénéficiaire, puis 20 % jusqu'à 700 000 euros et 31,25 % au-delà","Abattement global de 30 500 euros, puis droits de succession"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,1],[1,2],[2,0]]}$academy_deck$::jsonb, $academy_deck$L'âge de versement des primes et la clause bénéficiaire changent la fiscalité : à lire dans le bilan.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 29, $academy_deck$association$academy_deck$, $academy_deck$Relais professionnels$academy_deck$, 1, $academy_deck${"enonce":"Associez chaque professionnel à son rôle dans une transmission.","gauche":["Notaire","Avocat fiscaliste","Expert-comptable","Conseiller Entasis"],"droite":["Tient la stratégie d'ensemble : note, réunion commune, suivi du calendrier","Rédige les actes authentiques : donations, testament authentique, règlement de la succession","Sécurise les montages complexes, intervient en cas de contrôle ou de contentieux","Connaît les comptes de l'entreprise et prépare sa valorisation"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,1],[1,2],[2,3],[3,0]]}$academy_deck$::jsonb, $academy_deck$Coordonner plutôt que distribuer : chacun fait son métier, personne ne refait celui de l'autre.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 38, $academy_deck$ordre$academy_deck$, $academy_deck$Posture du conseiller$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre le travail du conseiller coordinateur sur un dossier de transmission.","elements":["Rédiger la note d'une page pour les professionnels, avec l'accord écrit du client","Établir le bilan sur les six volets","Organiser la réunion commune et tenir le compte rendu","Suivre le calendrier","Recueillir et reformuler les objectifs de la famille"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[4,1,0,2,3]}$academy_deck$::jsonb, $academy_deck$Préparer, transmettre, suivre : objectifs, bilan, note, réunion commune, calendrier.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 39, $academy_deck$ordre$academy_deck$, $academy_deck$Posture du conseiller$academy_deck$, 3, $academy_deck${"enonce":"Cas fictif : les époux Marchetti attendent une réponse immédiate sur la donation de leur maison. Remettez dans l'ordre la formulation de la prochaine étape.","elements":["Poser la date du second rendez-vous, dans quinze jours","Annoncer le relais notaire préparé par le cabinet, sans promettre de chiffre ni d'économie fiscale","Expliquer la méthode : l'écoute, l'étude, l'accompagnement","Demander les pièces avec leur raison : titres de propriété, justificatif du don au fils, relevé et clause du contrat","Reformuler l'objectif : transmettre la maison en gardant son usage, sans déséquilibre entre les trois enfants"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[4,2,3,0,1]}$academy_deck$::jsonb, $academy_deck$Objectif reformulé, méthode expliquée, pièces demandées avec leur raison, date posée, relais annoncé, aucune économie promise.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
