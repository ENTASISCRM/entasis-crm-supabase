...
$academy_deck$, competence = coalesce(nullif($academy_deck$Recueillir objectifs, horizon, situation, liquidité et tolérance au risque$academy_deck$, ''), competence), updated_at = now() where id = v_ver;
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 22, $academy_deck$ordre$academy_deck$, $academy_deck$Conduite de l'entretien$academy_deck$, 1, $academy_deck${"enonce":"Remettez dans l'ordre les étapes de la méthode Entasis, du premier entretien à la restitution.","elements":["Écouter la situation, les projets et le rapport au risque","Consigner dans le CRM les objectifs, l'horizon et la priorité","Annoncer au client la suite du parcours","Mener l'étude avec une simulation chiffrée","Remettre une feuille de route argumentée"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3,4]}$academy_deck$::jsonb, $academy_deck$L'écoute vient d'abord, puis l'étude chiffrée, puis la feuille de route ; rien n'est recommandé au premier échange.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 23, $academy_deck$ordre$academy_deck$, $academy_deck$Situation financière du client$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre les trois questions à poser en rendez-vous pour cerner la situation et le besoin de liquidité, puis ce que l'on fait des réponses.","elements":["« Que vous reste-t-il en fin de mois ? »","« Quelles dépenses importantes voyez-vous venir dans les deux ans ? »","« Si vous deviez sortir la moitié de cette somme dans six mois, pour quelle raison serait-ce ? »","Noter les réponses avec les montants"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3]}$academy_deck$::jsonb, $academy_deck$Reste en fin de mois, dépenses à deux ans, scénario de sortie à six mois, puis les montants notés.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 24, $academy_deck$ordre$academy_deck$, $academy_deck$Objectifs et horizon$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre la conduite du recueil des objectifs au premier entretien.","elements":["Ouvrir par une question large : « Qu'est-ce que cet argent doit vous permettre de faire, et quand ? »","Reformuler chaque projet avec une date","Classer les projets par priorité avec le client, pas à sa place","Demander ce qui se passerait si un projet devait être avancé"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3]}$academy_deck$::jsonb, $academy_deck$Question large, reformulation datée, priorisation par le client, puis test d'un projet avancé.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 25, $academy_deck$ordre$academy_deck$, $academy_deck$Situation financière du client$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre la construction des poches avec un client qui vient de recevoir une somme.","elements":["Constituer une épargne de précaution disponible et sûre","Dimensionner une poche pour les projets datés","Réserver seulement le reliquat au long terme","Examiner, lors de l'étude, les enveloppes possibles sur ce reliquat"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3]}$academy_deck$::jsonb, $academy_deck$Précaution d'abord, projets datés ensuite, long terme avec le reliquat ; l'étude vient après.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 26, $academy_deck$association$academy_deck$, $academy_deck$Tolérance et capacité de perte$academy_deck$, 1, $academy_deck${"enonce":"Associez chaque notion à sa définition.","gauche":["Tolérance au risque","Capacité à subir des pertes","Horizon","Objectif"],"droite":["Ce que le client accepte : voir son épargne baisser sans vendre","Ce que sa situation autorise sans remettre en cause un projet","La date, même approximative, attachée à un projet","Le projet de vie derrière la demande d'entrée"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$Tolérance et capacité sont deux informations distinctes ; l'objectif et son horizon les précèdent.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 27, $academy_deck$association$academy_deck$, $academy_deck$Liquidité des enveloppes$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque poche ou enveloppe à sa règle de disponibilité.","gauche":["PER individuel","Contrat d'assurance vie","Parts de SCPI","Épargne de précaution"],"droite":["Bloqué en principe jusqu'à la retraite, hors cas limitativement prévus","Rachat possible, versement sous deux mois au plus, fiscalité selon l'ancienneté","Revente sur un marché où l'acheteur n'est pas garanti, délai pouvant s'allonger","Disponible et sûre, repère AMF de 2 à 6 mois de revenus"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$Disponible ne veut pas dire immédiat : chaque enveloppe a son délai, ses conditions et sa fiscalité.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 28, $academy_deck$association$academy_deck$, $academy_deck$Obligations avant conseil$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque texte ou guide à ce qu'il établit.","gauche":["Code monétaire et financier, article L541-8-1","Code des assurances, article L522-5","Guide AMF « Épargner à son rythme » (2023)","Guide AMF « Investir votre épargne : étape par étape »"],"droite":["Le conseiller en investissements financiers recueille les informations avant tout conseil, sinon s'abstient","Les exigences et besoins du souscripteur sont précisés par écrit avant une assurance vie ou un contrat de capitalisation","Constituer d'abord une épargne de précaution disponible et sûre, de 2 à 6 mois de revenus","L'objectif oriente le choix du placement et l'horizon est une période sans besoin de la somme"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$Deux articles fixent les obligations, deux guides AMF donnent les repères de méthode.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 29, $academy_deck$association$academy_deck$, $academy_deck$Conduite de l'entretien$academy_deck$, 3, $academy_deck${"enonce":"Associez chaque erreur fréquente à la bonne pratique qui la corrige.","gauche":["Annoncer « une volatilité de 15 % »","Se contenter d'une case « dynamique » cochée","Noter un horizon « long terme »","Retenir les montants que le client cite de mémoire"],"droite":["Dire « 120 000 euros qui peuvent valoir 96 000 euros pendant plusieurs mois »","Demander ce qu'il a fait lors d'une baisse passée, montants à l'appui","Reformuler le projet avec une date, même approximative","Demander relevés, tableaux d'amortissement et avis d'imposition"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$Le risque se dit en euros et en durée, l'expérience vécue prime, l'horizon est daté et la situation est justifiée.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
