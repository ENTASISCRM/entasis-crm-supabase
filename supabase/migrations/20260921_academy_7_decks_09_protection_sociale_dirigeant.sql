...
- Aucune solution au premier entretien.$academy_deck$, competence = coalesce(nullif($academy_deck$Identifier les besoins de prévoyance, de santé et de protection des revenus d'un dirigeant$academy_deck$, ''), competence), updated_at = now() where id = v_ver;
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 22, $academy_deck$ordre$academy_deck$, $academy_deck$Posture en rendez-vous$academy_deck$, 1, $academy_deck${"enonce":"Remettez dans l'ordre les étapes de la méthode du cabinet face à un dirigeant qui demande un PER.","elements":["Chiffrer ce que verserait le régime obligatoire en cas d'arrêt de travail","Auditer les couvertures existantes et repérer l'écart avec le besoin","Étudier le PER dans l'étude, avec une simulation chiffrée","Identifier son régime social à partir de la forme juridique et du capital","Vérifier l'épargne de sécurité"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[3,0,1,4,2]}$academy_deck$::jsonb, $academy_deck$Régime, chiffrage, audit, épargne de sécurité, puis PER : la prévoyance est la fondation, le patrimoine vient ensuite.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 23, $academy_deck$ordre$academy_deck$, $academy_deck$Plafonds des IJ 2026$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre chronologique ce que prévoit le régime obligatoire pour un professionnel libéral arrêté plusieurs mois.","elements":["À partir du 4e jour : IJ de l'Assurance Maladie, au plus 197,51 € bruts par jour","Jours 1 à 3 : aucune indemnité journalière, hors exceptions","Ensuite : relais éventuel de la caisse de retraite professionnelle, à vérifier sur la source officielle","Au 90e jour : fin de l'arrêt indemnisé par l'Assurance Maladie"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[1,0,3,2]}$academy_deck$::jsonb, $academy_deck$Trois jours non indemnisés, IJ du 4e jour, arrêt de l'Assurance Maladie au 90e jour, puis relais à vérifier auprès de la caisse.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 24, $academy_deck$ordre$academy_deck$, $academy_deck$Conditions des IJ$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre les étapes du calcul de l'ordre de grandeur des IJ d'un artisan, tel que le conseiller l'écrit devant le client.","elements":["Limiter ce revenu au plafond annuel de la Sécurité sociale, 48 060 € en 2026","Multiplier par le nombre de jours d'arrêt, moins les trois jours non indemnisés","Comparer le résultat au revenu habituel et aux charges fixes","Diviser par 730 pour obtenir l'indemnité journalière estimée","Relever le revenu d'activité annuel moyen des trois dernières années sur les avis d'imposition"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[4,0,3,1,2]}$academy_deck$::jsonb, $academy_deck$Revenu moyen sur trois ans, plafonné, divisé par 730, multiplié par les jours indemnisés, comparé aux charges : la soustraction fait comprendre le besoin.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 25, $academy_deck$ordre$academy_deck$, $academy_deck$Posture en rendez-vous$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre les gestes du conseiller pour établir le statut social d'un dirigeant en rendez-vous.","elements":["Proposer de le vérifier avec l'expert-comptable du client avant le second rendez-vous","Si la structure est complexe (holding, plusieurs mandats), noter le point sans trancher seul","Demander les statuts ou le Kbis, la répartition du capital et la dernière pièce de cotisations","Reformuler à voix haute le régime dont relève le dirigeant"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[2,3,1,0]}$academy_deck$::jsonb, $academy_deck$Documents, reformulation, point noté si la structure est complexe, puis vérification avec l'expert-comptable : le conseiller ne tranche pas seul.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 26, $academy_deck$association$academy_deck$, $academy_deck$Statut social du dirigeant$academy_deck$, 1, $academy_deck${"enonce":"Associez chaque notion à sa définition.","gauche":["Travailleur indépendant","Assimilé salarié","Collège de gérance majoritaire","Parts attribuées au gérant"],"droite":["Parts du conjoint, du partenaire de PACS et des enfants mineurs non émancipés","Président de SAS ou gérant minoritaire de SARL, au régime général sans assurance chômage","Entrepreneur individuel, gérant majoritaire d'EURL ou de SARL, associé de SNC","Ensemble des gérants d'une SARL détenant plus de 50 % des parts"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,2],[1,1],[2,3],[3,0]]}$academy_deck$::jsonb, $academy_deck$Le statut social découle de la forme juridique et du capital, selon la règle du collège de gérance à plus de 50 %.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 27, $academy_deck$association$academy_deck$, $academy_deck$Plafonds des IJ 2026$academy_deck$, 1, $academy_deck${"enonce":"Associez chaque repère publié par ameli.fr pour 2026 à son montant.","gauche":["Plafond annuel de la Sécurité sociale","IJ maximale de l'artisan ou du commerçant","IJ maximale du professionnel libéral","Revenu moyen en dessous duquel l'IJ de l'artisan ou du commerçant est nulle"],"droite":["197,51 € bruts par jour","65,84 € bruts par jour","48 060 €","4 582 €"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,2],[1,1],[2,0],[3,3]]}$academy_deck$::jsonb, $academy_deck$Plafond 48 060 €, IJ maximale 65,84 € pour l'artisan et 197,51 € pour le libéral, IJ nulle sous 4 582 € de revenu moyen.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 28, $academy_deck$association$academy_deck$, $academy_deck$Posture en rendez-vous$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque document demandé au dirigeant à ce qu'il permet de vérifier.","gauche":["Échéancier Urssaf","Bulletin de paie","Trois derniers avis d'imposition","Conditions particulières d'un contrat de prévoyance"],"droite":["Cotisations de l'assimilé salarié","Base de calcul des indemnités journalières","Montant garanti, franchise, durée et définition de l'invalidité","Cotisations du travailleur indépendant"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,3],[1,0],[2,1],[3,2]]}$academy_deck$::jsonb, $academy_deck$La pièce de cotisations dépend du régime, les avis d'imposition fondent la base des IJ, les conditions particulières révèlent les garanties.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 29, $academy_deck$association$academy_deck$, $academy_deck$Audit des couvertures$academy_deck$, 1, $academy_deck${"enonce":"Associez chaque prestation ou notion du régime obligatoire à sa valeur selon ameli.fr.","gauche":["Capital décès de l'artisan ou du commerçant cotisant (2026)","Capital orphelin (2026)","Capital décès forfaitaire du salarié (depuis le 1er avril 2026)","Invalidité"],"droite":["9 612 €","2 403 € par enfant","Capacité de travail ou de gain réduite d'au moins deux tiers","4 009 €"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,3],[3,2]]}$academy_deck$::jsonb, $academy_deck$Le socle obligatoire reste modeste : 9 612 € pour l'artisan cotisant, 2 403 € par enfant, 4 009 € pour le salarié.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
