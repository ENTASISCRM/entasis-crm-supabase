...
$academy_deck$, competence = coalesce(nullif($academy_deck$Dérouler les sept étapes du rendez vous d’audit patrimonial standard, dans l’ordre et dans le temps imparti$academy_deck$, ''), competence), updated_at = now() where id = v_ver;
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 13, $academy_deck$multi$academy_deck$, $academy_deck$Les documents à récupérer$academy_deck$, 1, $academy_deck${"enonce":"Coche les documents à récupérer selon la trame.","choix":["Dernier avis d’imposition","Relevés d’assurance vie, PER, PEA, comptes titres","Carte d’identité du conjoint","Bilan et liasse fiscale si entrepreneur","Relevé de carrière","Ticket de caisse du dernier achat"]}$academy_deck$::jsonb)
  returning id into v_item;
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 19, $academy_deck$choix$academy_deck$, $academy_deck$Les durées$academy_deck$, 1, $academy_deck${"enonce":"Quelle est l’étape la plus longue ?","choix":["Situation personnelle et familiale","Patrimoine actuel","Objectifs du client","Documents à récupérer"]}$academy_deck$::jsonb)
  returning id into v_item;
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 42, $academy_deck$vrai_faux$academy_deck$, $academy_deck$Les documents à récupérer$academy_deck$, 2, $academy_deck${"enonce":"Si le client n’a pas de tableau d’actifs, on ne demande rien d’autre."}$academy_deck$::jsonb)
  returning id into v_item;
