-- ═══════════════════════════════════════════════════════════════════════════
-- Rentabilite : rendre les chiffres vrais.
--
-- Quatre corrections de fond.
--
-- 1. Une personne pouvait avoir plusieurs contrats dans l annee (alternance
--    puis CDI). L ancienne version n en retenait qu un seul, le plus recent :
--    les neuf mois d alternance disparaissaient du cout. On additionne
--    desormais tous les contrats qui couvrent l annee, plafonnes a douze mois.
--
-- 2. Les gerants etaient exclus du calcul. Ils reviennent, sur leur propre
--    ligne, avec leur remuneration reelle prise dans pnl_couts_personne. Ils
--    ne portent pas de part de frais fixes : leur remuneration EST deja dans
--    ces frais fixes, la leur imputer reviendrait a la compter deux fois.
--
-- 3. La retrocession versee au signataire (colonne COM SIGNATAIRE du grand
--    livre) n etait comptee nulle part. C est pourtant le seul cout d un
--    mandataire. Sans elle, un mandataire paraissait infiniment rentable.
--
-- 4. La commission attendue etait ignoree. Elle est desormais rendue a part,
--    jamais melangee a l encaisse : on doit pouvoir lire ce qui est en banque.
-- ═══════════════════════════════════════════════════════════════════════════

alter table public.production_encaissee
  add column if not exists retrocession numeric not null default 0;

comment on column public.production_encaissee.retrocession is
  'Part versee au signataire (COM SIGNATAIRE du grand livre). Cout du cabinet.';

-- Le type de retour change : il faut supprimer avant de recreer.
drop function if exists public.pnl_conseiller_annuel(integer);
...
returns table(
  ...
  cout_fixe numeric, cout_ecole numeric, cout_outils numeric,
  cout_frais_fixes numeric, cout_retrocession numeric, cout_total numeric,
...
begin
  ...
  --
  -- La seule chose a ecarter est le compte de test : profil desactive ET
  -- aucune remuneration. Quelqu un qui est parti garde un salaire, il reste
  -- compte pour les mois ou il etait la.
  select count(distinct c.profile_id) into nb_tetes
  ...
  with contrats as (
    ...
    select
      c.profile_id, c.full_name, c.type_contrat, c.date_debut,
      (greatest(0,
      ...
      coalesce(c.salaire_brut_mensuel, 0) * (1 + case upper(c.type_contrat)
      ...
      + case when coalesce(c.salaire_brut_mensuel, 0) > 0 then prm.mutuelle_mensuelle else 0 end
      as cout_mensuel
    from public.conseiller_contrats c
    join public.profiles pf on pf.id = c.profile_id
  ...
  couts as (
    select p.profile_id, p.full_name, p.type_contrat, p.est_gerant,
      p.mois_actifs, p.cout_fixe,
...
-- Le navigateur ne doit jamais pouvoir appeler cette fonction.
