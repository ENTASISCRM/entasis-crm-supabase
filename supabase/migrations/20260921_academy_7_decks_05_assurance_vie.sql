...
- Bénéficiaire acceptant : rien sans son accord.$academy_deck$, competence = coalesce(nullif($academy_deck$Expliquer enveloppe, supports, frais, rachats et principes de transmission$academy_deck$, ''), competence), updated_at = now() where id = v_ver;
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 3, $academy_deck$association$academy_deck$, $academy_deck$Enveloppe et supports$academy_deck$, 1, $academy_deck${"enonce":"Associez chaque élément du contrat à ce qu'il apporte.","gauche":["Enveloppe","Fonds en euros","Unités de compte"],"droite":["Contrat avec l'assureur qui apporte la souplesse : versements, arbitrages, rachats","Capital garanti par l'assureur, net des frais prévus au contrat","Nombre de parts garanti, valeur qui suit les marchés"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2]]}$academy_deck$::jsonb, $academy_deck$L'enveloppe donne la souplesse ; le fonds en euros garantit le capital, les unités de compte seulement un nombre de parts.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 9, $academy_deck$ordre$academy_deck$, $academy_deck$Enveloppe et supports$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre la démarche en rendez-vous avant de parler de supports.","elements":["« Quand aurez-vous besoin de ces sommes ? »","« Quelle part devez-vous pouvoir retrouver intacte ? »","« Quelle baisse temporaire pourriez-vous supporter sans racheter ? »","Reformuler : retrait possible à tout moment, mais à la valeur du jour des unités de compte","Remettre la note d'information et le document d'informations clés"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3,4]}$academy_deck$::jsonb, $academy_deck$Trois questions dans l'ordre (horizon, part intacte, baisse supportable), puis la reformulation, puis les documents précontractuels.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 13, $academy_deck$association$academy_deck$, $academy_deck$Frais du contrat$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque famille de frais au moment où elle s'applique.","gauche":["Frais sur versement","Frais de gestion du contrat","Frais d'arbitrage","Frais du support"],"droite":["À l'entrée, sur chaque somme versée","Chaque année sur l'épargne, fonds en euros compris","Quand le client change de support","Prélevés à l'intérieur de chaque unité de compte"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$Versement à l'entrée, gestion chaque année, arbitrage au changement de support, frais du support dans l'unité de compte.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 18, $academy_deck$ordre$academy_deck$, $academy_deck$Rachat et délais$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre chronologique la vie d'un contrat, de la souscription au rachat.","elements":["Le client est informé de la conclusion du contrat","Le délai de renonciation de 30 jours calendaires court","L'assureur adresse chaque année un relevé de situation avec les frais prélevés","Le client envoie une demande de rachat avec le dossier complet","L'assureur paie sous deux mois au plus","Passé ce délai, des intérêts de retard courent"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3,4,5]}$academy_deck$::jsonb, $academy_deck$Information, renonciation sous 30 jours, relevé annuel, demande de rachat, paiement sous deux mois, puis intérêts de retard.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 19, $academy_deck$association$academy_deck$, $academy_deck$Rachat et délais$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque repère à sa durée ou à sa date.","gauche":["Délai de renonciation","Délai maximal de paiement d'un rachat","Ancienneté du contrat ouvrant l'abattement annuel","Date charnière des versements pour les taux de 12,8 % et 7,5 %"],"droite":["30 jours calendaires","Deux mois à réception du dossier complet","Huit ans","27 septembre 2017"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$Renonciation 30 jours, paiement deux mois, abattement après huit ans, régime des versements depuis le 27 septembre 2017.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 24, $academy_deck$ordre$academy_deck$, $academy_deck$Fiscalité du rachat$academy_deck$, 3, $academy_deck${"enonce":"Cas fictif. Un client rachète 30 000 euros sur un contrat de onze ans, alimenté par des versements postérieurs au 27 septembre 2017 et inférieurs à 150 000 euros. Remettez dans l'ordre les étapes du traitement fiscal de ce rachat.","elements":["L'assureur calcule au prorata la part de gains comprise dans le rachat","Le capital versé compris dans le rachat revient sans impôt","L'abattement annuel de 4 600 ou 9 200 euros s'impute sur les gains","Le solde des gains supporte 7,5 %, ou le barème progressif sur option","Les prélèvements sociaux s'ajoutent sur les gains"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3,4]}$academy_deck$::jsonb, $academy_deck$Prorata des gains, capital hors impôt, abattement annuel, 7,5 % sur le solde, prélèvements sociaux en plus.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 27, $academy_deck$association$academy_deck$, $academy_deck$Fiscalité décès$academy_deck$, 1, $academy_deck${"enonce":"Associez chaque situation au régime des capitaux décès qui s'applique.","gauche":["Primes versées avant 70 ans","Primes versées après 70 ans","Conjoint marié ou partenaire de PACS","Primes manifestement exagérées"],"droite":["Abattement de 152 500 euros par bénéficiaire, puis 20 % et 31,25 %","Abattement global de 30 500 euros, puis droits de succession","Exonération, quel que soit l'âge au versement","Exception à l'article L132-13 : rapport et réduction redeviennent possibles"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$Le régime dépend de l'âge au versement ; le conjoint et le partenaire de PACS sont exonérés ; les primes exagérées sont requalifiables.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 40, $academy_deck$ordre$academy_deck$, $academy_deck$Clause bénéficiaire$academy_deck$, 3, $academy_deck${"enonce":"Remettez dans l'ordre la démarche du conseiller sur la clause bénéficiaire en rendez-vous.","elements":["Demander à voir la clause actuelle, pas seulement le nom du bénéficiaire","Poser la question : « Si vous disparaissiez demain, qui recevrait ce contrat ? »","Situer les régimes avant et après 70 ans en citant la source et sa date","Faire relire par le notaire toute clause complexe","Consigner la clause retenue au dossier et la réexaminer au point annuel"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3,4]}$academy_deck$::jsonb, $academy_deck$Lire la clause, questionner le souhait, situer les régimes, faire relire par le notaire, consigner et réexaminer.$academy_deck$);
end
$deck_assurance_vie$;
