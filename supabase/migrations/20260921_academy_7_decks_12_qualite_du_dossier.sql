...
$academy_deck$, competence = coalesce(nullif($academy_deck$Identifier pièces manquantes, incohérences et besoin d'escalade$academy_deck$, ''), competence), updated_at = now() where id = v_ver;
  ...
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 22, $academy_deck$ordre$academy_deck$, $academy_deck$Pièces du dossier$academy_deck$, 1, $academy_deck${"enonce":"Remettez dans l'ordre la gestion des pièces, du premier entretien à l'ouverture de l'étude.","elements":["Annoncer les pièces dès le premier entretien : « c'est la même règle pour tout le monde »","Envoyer la liste des pièces par écrit après le rendez-vous","Noter dans le CRM ce qui est reçu, ce qui manque et la date de chaque pièce","Ouvrir l'étude une fois l'identité et l'origine des fonds documentées"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3]}$academy_deck$::jsonb, $academy_deck$Annonce orale, liste écrite, suivi daté dans le CRM, puis étude seulement quand identité et origine des fonds sont prouvées.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 23, $academy_deck$ordre$academy_deck$, $academy_deck$Détecter les incohérences$academy_deck$, 2, $academy_deck${"enonce":"Remettez dans l'ordre le traitement d'une incohérence entre la fiche client et une pièce du dossier.","elements":["Surligner les écarts dans la fiche avant le rendez-vous","Aborder l'écart avec neutralité, comme un point de méthode","Reformuler la réponse du client et dire quelle pièce la documentera","Mettre à jour la fiche avec la version corrigée, la date et la source","Escalader si l'écart persiste sans explication crédible"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3,4]}$academy_deck$::jsonb, $academy_deck$Préparer, questionner sans soupçon, reformuler, écrire avec date et source, et n'escalader que si l'écart persiste.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 24, $academy_deck$ordre$academy_deck$, $academy_deck$Escalade et confidentialité$academy_deck$, 2, $academy_deck${"enonce":"Cas fictif. Un client refuse d'expliquer l'origine de 400 000 euros. Remettez dans l'ordre ce que fait le conseiller.","elements":["Clore le rendez-vous sans rien faire signer ni promettre","Noter dans le CRM les faits tels qu'observés","Transmettre par écrit au responsable désigné, le jour même","Attendre sa réponse avant tout nouveau contact sur ce dossier"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3]}$academy_deck$::jsonb, $academy_deck$Rien signé, faits notés, remontée écrite le jour même, puis silence sur le dossier jusqu'à la réponse du responsable.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 25, $academy_deck$ordre$academy_deck$, $academy_deck$Examen renforcé$academy_deck$, 3, $academy_deck${"enonce":"Remettez dans l'ordre la gradation des réactions face à un dossier qui pose problème, de la plus courante à la plus grave.","elements":["Pièce manquante ou écart explicable, réglé avec le client","Opération inhabituelle ou sans logique économique : examen renforcé","Soupçon : remontée écrite au responsable désigné","Déclaration à Tracfin par la personne habilitée"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"ordre":[0,1,2,3]}$academy_deck$::jsonb, $academy_deck$Premier niveau avec le client, deuxième niveau examen renforcé, troisième niveau soupçon remonté, puis déclaration par la personne habilitée.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 26, $academy_deck$association$academy_deck$, $academy_deck$Pièces du dossier$academy_deck$, 1, $academy_deck${"enonce":"Associez chaque article du Code monétaire et financier à l'obligation qu'il pose.","gauche":["Article L561-5","Article L561-5-1","Article L561-6","Article L561-8"],"droite":["Vérifier l'identité du client sur un document écrit à caractère probant","Recueillir l'objet et la nature de la relation d'affaires","Tenir la connaissance du client à jour et exercer une vigilance constante","N'exécuter aucune opération si ces éléments ne peuvent être obtenus"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$L561-5 identité, L561-5-1 objet de la relation, L561-6 vigilance constante, L561-8 aucune opération sans ces éléments.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 27, $academy_deck$association$academy_deck$, $academy_deck$Escalade et confidentialité$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque article du Code monétaire et financier à ce qu'il prévoit en matière de soupçon.","gauche":["Article L561-10-2","Article L561-15","Article L561-18","Article R561-23"],"droite":["Examen renforcé des opérations complexes, inhabituelles ou sans justification économique","Déclaration à Tracfin des sommes soupçonnées de provenir d'une infraction","Confidentialité de la déclaration : interdiction d'en révéler l'existence au client","Désignation des dirigeants ou préposés habilités à déclarer"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$L561-10-2 examen renforcé, L561-15 déclaration, L561-18 confidentialité, R561-23 personnes habilitées.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 28, $academy_deck$association$academy_deck$, $academy_deck$Détecter les incohérences$academy_deck$, 3, $academy_deck${"enonce":"Cas fictif. Vous reprenez le dossier d'un collègue parti. Associez chaque écart à son classement.","gauche":["Marié, deux enfants dans la fiche ; célibataire sur l'avis d'imposition","140 000 euros de revenus dans la fiche ; 92 000 euros sur l'avis de l'année précédente","310 000 euros reçus d'une SCI inconnue du dossier, client pressé"],"droite":["Erreur probable ou changement récent, à vérifier","Point à questionner, une pièce récente doit expliquer l'écart","Signal d'alerte, examen renforcé"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2]]}$academy_deck$::jsonb, $academy_deck$La situation familiale est une erreur probable, l'écart de revenus un point à questionner, l'origine des fonds inconnue un signal d'alerte.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
  values (v_ver, 29, $academy_deck$association$academy_deck$, $academy_deck$Pièces du dossier$academy_deck$, 2, $academy_deck${"enonce":"Associez chaque élément à connaître à la pièce qui le documente.","gauche":["Identité de la personne","Revenus et fiscalité","Origine de fonds issus d'une vente immobilière","Société cliente"],"droite":["Pièce d'identité en cours de validité","Dernier avis d'imposition","Acte de vente ou attestation notariée","Extrait d'immatriculation récent, statuts et bénéficiaires effectifs"]}$academy_deck$::jsonb)
  returning id into v_item;
  insert into public.academy_items_corriges (item_id, corrige, explication) values (v_item, $academy_deck${"paires":[[0,0],[1,1],[2,2],[3,3]]}$academy_deck$::jsonb, $academy_deck$Chaque information importante s'appuie sur une pièce datée : identité, avis d'imposition, acte notarié, extrait et bénéficiaires effectifs.$academy_deck$);
  insert into public.academy_items (version_id, ordre, type, competence, difficulte, payload)
