...
function AuthScreen() {
  ...
  // Recommandé : tous les conseillers Entasis ont un compte Google
  // @entasis-conseil.fr. Un seul login pour CRM + Lead Room (à terme).
  // Le paramètre `hd=entasis-conseil.fr` suggère le workspace pro à Google
  // (ne force pas, mais oriente le picker de compte). La restriction
  // stricte au domaine est faite côté serveur via trigger DB.
  async function handleGoogleLogin() {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      ...
      options: {
        ...
        queryParams: { hd: 'entasis-conseil.fr', prompt: 'select_account' },
      },
    })
    if (error) {
  ...
  // Magic link OTP — alternative au Google OAuth (qui nécessite un Client ID
  // configuré côté Supabase). L'user reçoit un lien dans son email, clique,
  // et est connecté direct sans mot de passe.
  async function handleMagicLink() {
    if (!email) {
      setMsg('Renseigne ton email pour recevoir un lien magique.')
      return
    }
    setLoading(true)
    setMsg('')
        ...
        {mode === 'login' ? (
          <>
            ...
            <div style={{
              ...
              margin: '12px 0', fontSize: 11, color: 'var(--t3)',
            ...
            {/* Lien magique par email — remplace l'ancien bouton Google */}
            <button
              className="w-full"
              style={{
                padding: '11px 20px', fontSize: 14, fontWeight: 600,
                background: '#fff', color: 'var(--t1)',
                border: '0.5px solid var(--bd-strong)', borderRadius: 8,
                cursor: loading ? 'not-allowed' : 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
                marginBottom: 16, transition: 'background 120ms ease',
              }}
              onMouseEnter={e => (e.currentTarget.style.background = '#F5F5F7')}
              onMouseLeave={e => (e.currentTarget.style.background = '#fff')}
              disabled={loading}
              onClick={handleMagicLink}
              title="On t'envoie un lien dans ton mail, tu cliques, t'es connecté"
            >
...
/* ─────────────────────────────────────────────────────────────────────────────
   ADVISOR DASHBOARD
───────────────────────────────────────────────────────────────────────────── */
function AdvisorDashboard({deals,objectifs,month,profile,onEdit,onGoTab,onGoView}){
    ...
    <div>
      ...
      <div className="advisor-hero">
        ...
        <div className="advisor-hero-kpis">
          ...
          {ppTarget>0&&<div className="advisor-hero-kpi"><div className="advisor-hero-kpi-label">{targetSource==='perso'?'Mon palier PP':'Objectif PP'}</div><div className="advisor-hero-kpi-value">{ppPct}%</div><div style={{fontSize:11,color:'var(--t3)',marginTop:2}}>signé · proj. {ppProjPct}%</div></div>}
...
/* ─────────────────────────────────────────────────────────────────────────────
   MANAGER DASHBOARD
───────────────────────────────────────────────────────────────────────────── */
function ManagerDashboard({deals,objectifs,month,teamProfiles,profile,onEdit,onGoView}){
      ...
      <div className="app-main">
        ...
        <div className="app-content">
          ...
          {/* onGoView : les KPI de l'accueil mènent à la liste qui les
              compose (phase 0 : tous étaient des impasses). */}
          {activeTab==='dashboard'&&(isManager?<ManagerDashboard deals={deals} objectifs={objectifs} month={month} teamProfiles={teamProfiles} profile={profile} onEdit={startEdit} onGoView={(tab,sub)=>{if(sub)setClientsVue(sub);setActiveTab(tab)}}/>:<AdvisorDashboard deals={deals} objectifs={objectifs} month={month} profile={profile} onEdit={startEdit} onGoTab={setActiveTab} onGoView={(tab,sub)=>{if(sub)setClientsVue(sub);setActiveTab(tab)}}/>)}
